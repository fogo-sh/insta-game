...
function buildGameEntry(key, config, state, startBlocked, catalog) {
  const c = config;
  const catalogEntry = catalog[key];
  return {
    ...state,
    displayName: c.displayName ?? catalogEntry?.displayName ?? key,
    connectAddress: c.connectPort ?? catalogEntry?.connectPort ? `${PUBLIC_HOST}:${c.connectPort ?? catalogEntry?.connectPort}` : null,
  ...
  app2.get("/status", async (c) => {
    ...
    const result = Object.fromEntries(
      Object.entries(games).map(([key, config]) => {
        ...
        return [key, buildGameEntry(key, config, state, hasPortConflict(config, occupied, key, games, cache2), gameCatalog)];
      })
    );
